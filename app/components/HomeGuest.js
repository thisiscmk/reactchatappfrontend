...
function HomeGuest() {
    ...
    switch (action.type) {
      ...
      case "emailUniqueResults":
        if (action.value) {
          ...
          draft.email.message = "This email already exists in our database"
        } else {
  ...
  //hook to check if username already exists in the database
  useEffect(() => {
  ...
  //hook to check if email already exists in the database
  useEffect(() => {
    if (state.email.checkCount) {
      ...
      async function fetchResults() {
        try {
          const response = await axios.post("/doesEmailExist", { username: state.email.value }, { cancelToken: request.token })
    ...
    if (state.submitCount) {
      ...
      async function fetchResults() {
        try {
          ...
          globalAppDispatch({ type: "flashMessage", value: "Congrats! Welcome to your new account" })
        } catch (e) {
          console.log("There was a problem or the request was cancelled")
        }
      }
  ...
  return (
    <Page wide={true} title="Home Guest">
